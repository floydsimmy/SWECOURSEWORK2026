...
Per the coursework specification, each five-member team has 100 peer-review points to distribute across its members. Marks reflect each member's contribution to the project as a whole. Marks are non-negative integers and sum to exactly 100. A score of 0 means a member made no contribution at all (a "ghost") and triggers an overall coursework grade of 0 for that member; the team unanimously confirms this is **not** the case for any member here.
...
- `SetupScreen` and `GameScreen` integration; `ScreenManager` wiring; `EndScreen` v1 (Sprint 2).
- `EndScreen` polish, hover states, `MessageBox`, message-history panel; Sprint 4 planning started in parallel (Sprint 3).
...
- Full validation suite, **F12 fix** (suspect/weapon location tracking), `validate_game_state`, `verify_deck`, `get_turn_summary`, `turn_history`, `reset_game` (Sprint 3).
...
- Sprint planning, scope decisions (grid + AI descope sign-off), `docs/decisions.md` entries, group-report skeleton (Sprint 2).
- Acceptance criteria for F9–F16, **F12 bug identified and added to Sprint 3 scope**, group-report skeleton expansion, system test report review (Sprint 3).
...
- UI component library — `Button`, `TextInput`, `DropdownMenu`, `MessageBox`; `GameScreen` layout (Sprint 2).
- `GameScreen` visual polish; hover states across `Button` instances; `MessageBox` rendering cleanup (Sprint 3).
...
- System tests F4–F8 in `docs/testing/system_test_report.md`; GitHub Issues bug log; in-sprint regression runs (Sprint 2).
- 28+ new system test cases for F9–F16 and NF2; full regression run; bug log audit (zero open critical bugs at sprint close); test-coverage report submitted via Discord ahead of the Sprint 3 review (Sprint 3).
- Final regression run on 27 April (100 of 100); system test report finalised; bug log closed; final regression on the unpacked submission ZIP — also 100 of 100, confirming clean export (Sprint 4).
