...
| F4  | The game shall allow a player to roll a six-sided die and move that number of squares along the 24 × 24 board, ending in either a corridor tile or a room reached via a door. |
| F4a | The game shall permit a player to enter a room only by ending their dice move on one of that room's defined door tiles, and shall not allow a player to enter and exit the same room within a single dice move. |
