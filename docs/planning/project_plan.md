...
Watson Games asked for a digital version of their *Clue!* board game with a Pygame GUI, 3–6 players, the full suggestion / refutation / accusation loop, the standard tile-based grid + dice movement, and an autonomous-player option. We delivered all of these as MVP across four sprints between 23 February and 29 April 2026 — see ADR-006 in `docs/decisions.md` for the Sprint 2 midpoint decision to keep grid + dice and AI in scope rather than descope them.
...
The shipped test count reflects the kept-in-scope decision: 116 unit tests passing at submission (33 from Sprint 1, ~30 added in Sprint 2, ~50 added in Sprint 3 covering F12 + validation + AI behaviour, stable at 113 through most of Sprint 4 with two additions during submission prep — a dice-fairness distribution test covering the spec §7 fair-dice integrity requirement and an engine-level guard against rolling twice in a single turn — bringing the final count to 116).
