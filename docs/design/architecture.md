...
1. **Testability.** The 116-test pytest suite runs in well under a second because no test touches SDL. If we'd entangled rules with rendering, every test would need a window — and the AI's full-turn simulation tests would not be possible at all.
...
3. **Three consumers, one engine.** The same engine API is called by the GUI, by the AI (`take_ai_turn`), and by the tests. That triangulation is what keeps the API honest. If a function works for two consumers but not the third, we've usually learned something useful.
