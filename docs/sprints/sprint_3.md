...
| Floyd        | Owned `EndScreen` polish (solution reveal, Play Again / Main Menu); button hover states across all screens; owned the message-history panel on `GameScreen`; `MessageBox` component refinement; **off-by-one fix on `legal_moves_for_roll`** carried over from Sprint 2; Sprint 4 planning started in parallel (submission pack structure, video planning).                                       |
| Maysarah     | Owned the validation suite — unknown suspect/weapon/room rejection, wrong-turn guards on every action, eliminated-cannot-act enforcement, action-after-game-over guards, draw / last-player-standing logic; led the implementation of the **F12 fix** (suspect/weapon location tracking added to `GameState`, populated in `make_suggestion`); owned `get_turn_summary`, `turn_history`, `reset_game`, `validate_game_state`, `verify_deck`; led the design and implementation of the **AI player module** (`src/game/ai.py`) — `RandomAIPlayerStrategy`, `take_ai_turn`, `run_ai_simulation`, `DetectiveNotes`, the privacy invariants, and the `tests/test_ai.py` file (13 tests). |
...
Sprint 3 was when the AI behaviour tests in `tests/test_ai.py` landed (13 tests covering full-turn, privacy, and refutation invariants). Sprint 3 also added validation tests (F9–F11, F13–F16), F12 token-position tests, reproducibility / typed-signature tests, and edge-case / full-game regression tests across the engine. All passing.
...
**Is there a working prototype?** Yes — full playable build with mixed Human / AI games, EndScreen, and complete edge-case handling. The pytest suite is green.
